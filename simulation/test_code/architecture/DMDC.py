...
class DMDC(nn.Module):
    ...
    def __init__(self, num_iterations=3, n_feat=28):
        ...
        self.spatial_all = SpatialAB_all(stage=0, dim=28, num_blocks=1)
        self.fution_all = SpatialAB_together(stage=0, dim=28)
...
# model = DMDC(num_iterations=9).cuda()
# input = torch.randn(1,28, 256, 256).cuda()
# Phi_batch = torch.randn(1, 28, 256, 256).cuda()
# Phi_s_batch = torch.randn(1,28, 256, 256).cuda()

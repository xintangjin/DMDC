...
class Sparsity_Estimator(nn.Module):
    def __init__(self, dim=31, expand=2, sparse=False):
        ...
        # Input projection
        self.in_proj = nn.Conv2d(31, dim, 1, 1, 0, bias=False)
...
class CST(nn.Module):
    def __init__(self, dim=31, stage=2, num_blocks=[2, 2, 2], sparse=False):
        ...
        self.fution = nn.Conv2d(31*2, 31, 1, 1, 0, bias=False)

        # Sparsity Estimator
        if num_blocks==[2,4,6]:
            self.fe = nn.Sequential(Sparsity_Estimator(dim=31,expand=2,sparse=False),
                        Sparsity_Estimator(dim=31, expand=2, sparse=sparse))
        else:
            self.fe = Sparsity_Estimator(dim=31, expand=2, sparse=sparse)
...
# mask = torch.randn(1,31, 256, 256).cuda()
...
# # print(z.shape)
# total_ops, total_params = profile(model, inputs=(input, mask))
# print(

...
class DAUHST(nn.Module):
    ...
    def __init__(self, num_iterations=1):
        ...
        self.para_estimator = HyPaNet(in_nc=31, out_nc=num_iterations*2)
        self.fution = nn.Conv2d(31*2, 31, 1, padding=0, bias=True)
        self.num_iterations = num_iterations
        self.denoisers = nn.ModuleList([])
        for _ in range(num_iterations):
            self.denoisers.append(
                HST(in_dim=32, out_dim=31, dim=31, num_blocks=[1,1,1]),
            )
...
# model = DAUHST(num_iterations=9).cuda()
# input = torch.randn(1,256, 316).cuda()
# Phi_batch = torch.randn(1, 31, 256, 316).cuda()
# Phi_s_batch = torch.randn(1, 256, 316).cuda()
